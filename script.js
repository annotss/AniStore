...
function updateView(currentBtnClassName, currentBtnParentID) {
    ...
    totalPricesObject[currentBtnParentID].innerHTML = totalPricesArray[currentBtnParentID].toFixed(2) + "&nbsp;DT";
}
...
function updateTotalPrice(){
    ...
    document.getElementById("total-Price").innerHTML = "Prix Total est = " + totalPrice.toFixed(2) + "&nbsp;DT";
}
...
function assignEventListenerImg(currentImg) {
    currentImg.addEventListener("click", function () {
        ...
        if(currentImgSize.includes("32")){
           currentImg.src = "res/Webp.net-resizeimage (2).jpg" ;
        }
        else{
